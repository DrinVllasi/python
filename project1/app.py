...
if st.button("Create Developer"):
    ...
    response = requests.post("http://localhost:800/developers/", json=dev_data)
...
if st.button("Create Project"):
    ...
    response = requests.post("htttp://localhost:8000/projects/", json=proj_data)
    st.json(response.json())
